...
export default async function VirtualGalleriesPage() {
    ...
    <div className="mx-auto flex w-full max-w-sm flex-1 flex-col gap-6 px-4 py-9">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h1 className="mt-2 text-xl font-semibold text-artego-black">Virtual Galleries</h1>
        </div>
        <Link
      ...
      {!projects || projects.length === 0 ? (
        <p className="text-base text-grey-600">
          No virtual galleries yet. Start one with &ldquo;+ New&rdquo;.
        </p>
      ) : (
        <ul className="flex flex-col gap-2">
          {projects.map((p) => (
            <li key={p.id}>
              <Link
                href={`/dashboard/virtual-gallery/${p.id}`}
